...
<h4 class="text-center" style="padding: 0;margin: 0">SCREENING PROKES {{ $role=='siswa'?'PESERTA DIDIK':($role=='guru'?'GURU DAN TENAGA KEPENDIDIKAN':'') }}</h4>
<h4 class="text-center" style="padding: 0;margin: 0">{{ strtoupper(getenv('INS_NAME')) }}</h4>
  ...
  <tbody>
    ...
    @foreach ($data as $key => $v)
      @php
        ...
        if (!@$v['datang']->suhu || !@$v['pulang']->suhu || @$v['datang']->suhu>=$kegiatan->max_temp||@$v['pulang']->suhu>=$kegiatan->max_temp||!@$v['datang']->prokes||!@$v['pulang']->prokes?'text-danger':'') {
          $tidak_memenuhi++;
        }
      @endphp
      <tr class="{{ @$v['datang']->suhu>=$kegiatan->max_temp||@$v['pulang']->suhu>=$kegiatan->max_temp||!@$v['datang']->prokes||!@$v['pulang']->prokes?'text-danger':'' }}">
        ...
        @endif
        <td class="text-center">{{ @$v['datang']->suhu?(@$v['datang']->prokes?'Ya':(!is_null(@$v['datang']->prokes)?'Tidak':'-')):'-' }}</td>
        ...
        <td class="text-center">{{ @$v['pulang']->suhu?(@$v['pulang']->prokes?'Ya':(!is_null(@$v['pulang']->prokes)?'Tidak':'-')):'-' }}</td>
        ...
        <td>
          @if (@$v['datang']->keterangan)
            <strong>Datang: </strong>{{ @$v['datang']->keterangan }}
          @endif
          @if (@$v['pulang']->keterangan)
            <br>
            <strong>Pulang: </strong>{{ @$v['pulang']->keterangan }}
          @endif
      </tr>
    @endforeach
  </tbody>
      ...
      <tr>
        <td style="padding-left: 14px">
          <table>
            ...
            <tr>
              <td style="padding: 0;padding-top: 65px;font-weight: bold;border-bottom: solid 1px #000">{{ @$config->kepsek }}</td>
            </tr>
            <tr>
        ...
        <td>
          <table>
            ...
            <tr>
              <td style="padding: 0;padding-top: 65px;font-weight: bold;border-bottom: solid 1px #000">{{ @$config->satgas }}</td>
            </tr>
            <tr>
        ...
        <td>
          <table>
            ...
            <tr>
              <td style="padding: 0;padding-top: 65px;font-weight: bold;border-bottom: solid 1px #000">{{ @$config->petugas }}</td>
            </tr>
            <tr>
              <td style="padding: 0;">NIP. {{ @$config->petugas_nip }}</td>
            </tr>
          </table>
        </td>
      </tr>
